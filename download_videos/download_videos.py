...
class download_videos(object):

    def __init__(self):
        self.vip_parser = "http://cdn.yangju.vip/kc/api.php"
        self.ts = []
        self.download_path = os.getcwd() + '/ts/'
        self.filename = "m3u8"
        self.key = ""

    def downloader(self,surl):
        """
        step1 准备下载文件夹
        """
        #检查是否存在下载文件夹
        ...
        """
        #使用vip解析网站请求视频地址
        req_vip = requests.post(url = self.vip_parser, data={"url":surl})
        html = req_vip.text
        #将返回内容转换为json格式
        ...
        if not os.path.exists(os.getcwd()+self.filename):
            #如果不存在则下载最新的（关闭SSL认证）
            req_video = requests.get(url = m3u8_url, verify = False)
        ...
        key_url = m3u8_url.replace('index.m3u8',key_uri)
        #请求key
        req_key = requests.get(url = key_url)
        #获取返回内容，返回的是二进制的密码
        key = req_key.content
        ...
        for i in range(5):
            ...
            ts_name = self.ts[i]
            #获取ts个数
            ts_id = int(ts_name[8:11])
            #拼接ts地址
            ts_url = m3u8_url.replace("index.m3u8",ts_name)
            #调用ts地址
            req_ts = requests.get(url = ts_url)
            #将ts文件下载到本地文件夹
...
if __name__ == "__main__":
    dv = download_videos()
    dv.downloader("http://www.iqiyi.com/v_19rrmepy6o.html")
